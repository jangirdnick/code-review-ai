...
export async function indexCodebase(
   ...
   for(const file of files) {
    ...
    const truncatedContent = content.slice(0, 8000) // Pinecone has a max vector size, so we need to truncate the content
   ...
   if(vectors.length > 0){
    const batchSize = 100; // Pinecone recommends batching for better performance
